...
LATEST_LINK="outputs/revision_8gpu_logs/pdformer_resdiff_hetero_latest"
if [[ "${1:-launch}" == "launch" ]]; then
  LOG_ROOT="outputs/revision_8gpu_logs/pdformer_resdiff_hetero_$(date +%Y%m%d_%H%M%S)"
  mkdir -p "$LOG_ROOT"
...
case "${1:-launch}" in
  launch)
    run_one "${GPU0:-0}" configs/pems08_pdformer_resdiff_hetero.yaml 1e-3 full_hetero_lr1e3
    run_one "${GPU1:-1}" configs/pems08_pdformer_resdiff_hetero_nosem.yaml 1e-3 nosem_hetero_lr1e3
    run_one "${GPU2:-2}" configs/pems08_pdformer_resdiff_hetero.yaml 5e-4 full_hetero_lr5e4
    run_one "${GPU3:-3}" configs/pems08_pdformer_resdiff_hetero_nosem.yaml 5e-4 nosem_hetero_lr5e4
    echo "[logs] ${LOG_ROOT}"
    ;;
  status)
    echo "[logs] ${LOG_ROOT}"
  ...
  tail)
    ...
    ;;
  *)
    echo "Usage: bash scripts/launch_pdformer_resdiff_hetero_4gpu.sh [launch|status|tail]" >&2
    exit 1
    ;;
esac

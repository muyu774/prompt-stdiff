...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/run_pems04.sh [--mode train|eval|infer] [--config_file PATH]
                           [--horizon_steps 3|6|12] [--history_steps N] [--gpu_id 0-9] [--ckpt PATH] [--out PATH]
                           [--eval_interval N] [--num_eval_samples N] [--train_num_eval_samples N]
                           [--max_eval_batches N] [--lr FLOAT] [--gamma FLOAT] [--save_tag STR]
...
if [[ -n "${CONFIG_FILE}" ]]; then
  if [[ "${CONFIG_FILE}" = /* ]]; then
    BASE_CFG="${CONFIG_FILE}"
  else
    BASE_CFG="${ROOT_DIR}/${CONFIG_FILE}"
  fi
else
...
default_save_dir = f"./outputs/checkpoints/${DATASET}_h{h}"
if "${CONFIG_FILE}" == "":
    save_dir = default_save_dir
else:
    save_dir = cfg["train"].get("save_dir", default_save_dir)
if save_tag != "":
...
print(str(cfg["train"]["save_dir"]) + "/best.pt")
PY
)"
  fi

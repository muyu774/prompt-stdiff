...
\begin{abstract}
Reliable traffic forecasting requires prediction intervals that are both calibrated and sharp, especially when forecasts are used for operational decisions under congestion, demand surges, and incident-driven regime shifts. Existing probabilistic traffic models are often evaluated by aggregate distributional scores, but these averages may hide systematic under-coverage in safety-critical regimes. This paper proposes \emph{Hierarchical Load-Conditioned Conformalized Quantile Regression} (\hlc), a deployable post-hoc calibration layer for probabilistic traffic forecasting. A strong deterministic spatio-temporal backbone is frozen and used as the mean forecast; \hlc\ then calibrates only the residual uncertainty, preserving the backbone's point MAE/RMSE by construction. The method organizes residual conformal corrections along a traffic-structured hierarchy---global, horizon, node, predicted-load bin, and time-of-day bin---and applies sample-size-adaptive shrinkage from fine cells toward their parents. This design retains the sharpness of fine-grained group-conditional calibration when calibration data are sufficient, while avoiding the sparse-cell instability of naive Mondrian conformal calibration. Experiments on PeMS08 and PeMS04 show that \hlc\ achieves the best interval score at near-nominal coverage among conformal residual calibration baselines, improves window-level Winkler score over an external CSDI interval anchor on matched windows, and remains robust when the calibration set is reduced to 5\%. Incident-regime diagnostics further show that aggregate metrics can obscure severe coverage failures on rare traffic drops. The results position \hlc\ as a simple, interpretable, and inference-efficient calibration layer for reliable probabilistic traffic forecasting, rather than as another heavy generative forecaster.
\end{abstract}
...
The paper is positioned as a reliable calibration algorithm for traffic forecasting rather than another diffusion architecture. We keep diffusion residuals, heteroscedastic Gaussian residuals, and semantic conditioning as baselines or diagnostic components, but the deployable method is \hlc. Experiments on PeMS08 and PeMS04 show that \hlc\ achieves the best Winkler interval score at near-nominal coverage among residual conformal calibration baselines, retains its advantage under calibration-data scarcity, and improves interval score over an external CSDI anchor on matched windows. We also provide incident-regime reliability diagnostics showing why aggregate probabilistic metrics alone are insufficient for ITS deployment.

The contributions are summarized as follows:
\begin{itemize}
    \item We formulate probabilistic traffic forecasting as \emph{mean-preserving residual calibration}: a frozen deterministic backbone supplies the point forecast, and the uncertainty layer calibrates residual intervals without changing MAE/RMSE.
    \item We propose \hlc, a hierarchical load-conditioned conformal calibration algorithm that combines traffic-structured conditioning (global $\rightarrow$ horizon $\rightarrow$ node $\rightarrow$ predicted-load $\rightarrow$ time-of-day) with sample-size-adaptive shrinkage.
    \item We show that \hlc\ achieves a strong calibration--sharpness trade-off on PeMS08 and PeMS04, obtaining the best Winkler score at near-nominal coverage among conformal residual calibration baselines and remaining robust as calibration data are reduced to 5\%.
    \item We compare \hlc\ with external generative probabilistic baselines on interval scores using matched test windows, while keeping CRPS-based distributional comparisons separate to avoid mixing interval calibration and sampling-based objectives.
    \item We introduce incident-regime reliability diagnostics, including event-conditioned coverage and a mean-vs-dispersion root-cause ratio, to reveal failures hidden by aggregate metrics and to distinguish variance miscalibration from frozen-mean regime surprises.
\end{itemize}
...
The proposed protocol freezes $f_\theta$ before residual calibration. The point forecast used for MAE and RMSE remains $\hat{\mu}$ for all residual calibration variants. Thus any probabilistic layer can only change the interval or distribution around $\hat{\mu}$, not the deterministic point estimate. This prevents uncertainty calibration from silently improving or degrading point accuracy and makes same-backbone comparisons interpretable.
...
\end{equation}
A positive value means that the base interval failed to cover the residual and must be widened; a negative value means the residual lies inside the base interval.
...
\end{equation}
Predicted-load bins are computed from $\hat{\mu}$ using validation-set quantiles and then fixed for test inference. This hierarchy reflects two traffic facts: residual uncertainty varies spatially and temporally, and roads under different predicted load regimes have different residual dispersion.
...
\end{equation}
All corrections are computed once on validation data. Test-time inference is table lookup, requiring $O(1)$ additional computation per target point after the deterministic backbone forecast.

\begin{figure*}[t]
\centering
\fbox{\parbox[c][1.45in][c]{0.92\textwidth}{\centering \textbf{Figure placeholder: Method framework.} Historical traffic $X$ $\rightarrow$ frozen deterministic backbone $\rightarrow$ mean forecast $\hat{\mu}$ $\rightarrow$ residual score construction $\rightarrow$ HLC-CQR hierarchy and shrinkage $\rightarrow$ calibrated interval $[L,U]$.}}
...
\begin{figure}[t]
\centering
\fbox{\parbox[c][1.55in][c]{0.43\textwidth}{\centering \textbf{Figure placeholder: HLC hierarchy.} Tree: global $\rightarrow$ horizon $\rightarrow$ node $\rightarrow$ predicted load $\rightarrow$ time of day, with $Q_g^*=\lambda_gQ_g+(1-\lambda_g)Q^*_{pa(g)}$.}}
...
Table~\ref{tab:hlc_tau} studies the shrinkage parameter $\tau$. The two extremes are undesirable: $\tau=0$ recovers naive fine-grained Mondrian calibration and is vulnerable to sparse cells, while $\tau\rightarrow\infty$ ignores fine traffic structure and reverts to coarse conformal calibration. Intermediate shrinkage gives the best interval score. We use $\tau=20$ as the main validation-registered setting; $\tau=50$ is reported only as a sensitivity point.

\begin{table}[t]
\centering
\caption{Shrinkage ablation (8-seed mean, nominal $90\%$). $\tau=0$ is naive fine-grained Mondrian calibration; $\tau=\infty$ is coarse conformal.}
\label{tab:hlc_tau}
\footnotesize
\begin{tabular}{lcccc}
\toprule
...
20 (main)        & 0.896 & 87.64 & 0.894 & 111.22 \\
50 (sensitivity) & 0.897 & \textbf{87.54} & 0.896 & \textbf{111.11} \\
...
Table~\ref{tab:hlc_sample} is the most direct evidence for the hierarchical shrinkage mechanism. At full calibration, load-conditioned, load$\times$TOD, and \hlc\ variants are close. When the calibration set is reduced, naive load$\times$TOD calibration loses sharpness quickly because fine cells become sparse. \hlc\ degrades gracefully and remains best at every calibration ratio on both datasets.
...
The advantage of \hlc\ is not specific to the 90\% interval. At 80/90/95\% nominal levels, \hlc\ achieves the best Winkler score among conformal, CQR-load, and \hlc\ on both datasets while maintaining near-nominal coverage; for example, PeMS08 Winkler scores are 68.74/87.64/109.93 and PeMS04 scores are 88.83/111.22/136.71. Using matched test windows, \hlc\ also improves window-level Winkler score over an external CSDI interval anchor on both datasets (PeMS08 $\Delta=-9.40$, 90\% CI $[-9.82,-8.99]$; PeMS04 $\Delta=-8.90$, 90\% CI $[-9.27,-8.54]$; Wilcoxon $p\approx 0$). This is an interval-score comparison, not a CRPS claim: \hlc\ is an interval calibration method rather than a sampler.

\subsection{Localized Conformal and Runtime}
A natural baseline is localized conformal prediction with continuous-load k-nearest-neighbor search. Using the same CQR score and selecting $k$ on a held-out validation split, localized conformal is competitive: \hlc\ is lower on PeMS08 (87.77 vs. 88.53 Winkler), while localized conformal is slightly lower on PeMS04 (110.65 vs. 111.24). The operational difference is runtime and interpretability. \hlc\ builds a single correction table and performs $O(1)$ lookup per test point, while localized conformal performs per-test neighbor search over the calibration pool, roughly two orders of magnitude slower in our implementation. Thus \hlc\ is not claimed to dominate generic locality, but to provide a strong deployable calibration--sharpness trade-off with transparent traffic cells.

\subsection{Incident-Regime Reliability}
Table~\ref{tab:event_refresh} shows that rare traffic drops remain difficult for all probabilistic methods. On the drop subset, coverage is extremely low for external generative baselines and residual generators; on spike windows, coverage is much stronger. This asymmetry motivates event-conditioned reliability reporting rather than relying only on aggregate metrics.
